            ...
            .await
        {
            ...
            Err(responses_err) => {
                    ...
                    && Self::should_fallback_to_chat_completions(&responses_err)
                {
                    ...
                    let responses_detail = format_transport_error(&responses_err);
                    return fallback_provider
                        .chat_with_history(messages, model, temperature)
                        .await
                        .map_err(|chat_err| {
                            let chat_detail = format_transport_error(&chat_err);
                            anyhow::anyhow!(
                                "{} Responses API failed: {responses_detail} (chat-completions fallback failed: {chat_detail})",
                                self.name
                            )
                        });
                }
